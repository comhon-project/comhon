...
class SerializationManifestFile extends AbstractManifestFile {
	...
	protected function _getModelName() {
		return 'Comhon\Serialization\File';
	}
	...
	protected function _getPath(UniqueObject $object) {
		...
		if (is_null($this->format)) {
			return ModelManager::getInstance()->getSerializationManifestPath($manifestPath, $fullyQualifiedNamePrefix, $fullyQualifiedNameSuffix);
		} else {
			$path_afe = ModelManager::getInstance()->getSerializationManifestPath($manifestPath, $fullyQualifiedNamePrefix, $fullyQualifiedNameSuffix);
			return dirname($path_afe).DIRECTORY_SEPARATOR.'serialization.'.$this->format;
		}
	}
