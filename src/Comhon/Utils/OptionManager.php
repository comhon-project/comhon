...
class OptionManager
{
	...
	public function registerOptionDesciption(array $opts)
	{
		...
		foreach ($opts as $name => $opt) {
			if (in_array($name, $names)) {
				throw new \Exception("error, several options with same name '$name'");
			}
			$names[] = $name;
			if (isset($opt['short'])) {
				if (in_array($opt['short'], $shorts)) {
					throw new \Exception("error, several options with same short name '{$opt['short']}'");
				}
				$shorts[] = $opt['short'];
			}
			if (isset($opt['long'])) {
				if (in_array($opt['long'], $longs)) {
					throw new \Exception("error, several options with same long name '{$opt['long']}'");
				}
				$shorts[] = $opt['long'];
			}
			...
			if (array_key_exists('required', $opt) && !$opt['has_value']) {
				throw new \Exception('error, required options must have \'has_value\' set to true');
			}
		}
	...
	public function getOptions()
	{
		...
		$tmp_options = getopt($shortopts, $longopts, $index);
		if ($index != $argc) {
			throw new \Exception('invalid arguments, please see help by typing -h or --help argument');
		}
		...
		foreach ($this->optionsDescription as $name => $opt) {
			...
			if (array_key_exists($short_option, $tmp_options)) {
				if (array_key_exists($long_option, $tmp_options)) {
					throw new \Exception('conflict between arguments \'' . $short_option . '\' and \'' . $long_option . "'" . PHP_EOL);
				}
		...
		foreach ($this->optionsDescription as $name => $opt) {
			$valid = (
					isset($options[$name])
					&& (!isset($opt['enum'])  || in_array($options[$name], $opt['enum']))
					&& (!isset($opt['pattern']) || preg_match("{$opt['pattern']}", $options[$name], $match))
				)
				|| (
					!isset($options[$name]) 
					&& (!isset($opt['required']) || !$opt['required'])
				);
			if (!$valid) {
					$filterType = null;
					$filter = null;
				if (isset($opt['enum'])) {
					$filterType = Cli::FILTER_VALUE;
					$filter = $opt['enum'];
				} elseif (isset($opt['pattern'])) {
					$filterType = Cli::FILTER_REGEX;
					$filter = [$opt['pattern']];
				}
				if (isset($options[$name])) {
					fwrite(Cli::$STDOUT, "\033[41mInvalid argument $name\033[0m".PHP_EOL);
				}
				$options[$name] = Cli::ask("please enter a $name.", null, $filter, $filterType);
			}
		}
